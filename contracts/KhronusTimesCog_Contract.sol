...
contract KhronusTimeCog {
    ...
    */
    ...
    event Debug (uint[3] basedate, uint[3] compareddate, uint baseTimestamp, uint baseDirection, uint comparedTimestamp, uint comparedDirection);
    ...
    */
    ...
    function getDayTimestamp(uint _year, uint _month, uint _day) public pure returns (uint _timestamp, uint _direction){
    ...
    function getDateObject(uint _timestamp, uint _direction) public pure returns (uint[5] memory _result) {
    ...
    function getDateObjectShort(uint _timestampDays, uint _direction) public pure returns (uint[3] memory _result) {
    ...
    //Time Delta
    function timeDelta(uint[3] memory _baseDate,uint[3] memory _comparedDate) public  pure returns (uint _timestampDays, uint _direction){
        ...
        //emit Debug(_baseDate, _comparedDate, baseT[0], baseT[1],comparedT[0],comparedT[1]);
    }
    ...
    function isLeapYear(uint _year) public pure returns(bool _result) {
        _result = _result = (_year % 4 == 0 &&( _year % 100 > 0 ||  _year % 400 == 0)) ? true:false;
    }
    
    function getDaysInMonth(uint _year,uint _month)public pure returns(uint _result) {
        uint8[12] memory daysInMonth = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
        _result = (isLeapYear(_year) && _month == 2) ? 29: daysInMonth[_month - 1];
    }

    function isValidDate(uint _year, uint _month, uint _day) public pure returns(bool _result) {
    ...
    function isValidTimestamp(uint _timestamp) public pure returns(bool _result) {
      return (_timestamp <= (LIMET_DAY_TIMESTAMP* 1 days));
    }

    function isValidDayTimestamp(uint _timestamp) public pure returns(bool _result) {
